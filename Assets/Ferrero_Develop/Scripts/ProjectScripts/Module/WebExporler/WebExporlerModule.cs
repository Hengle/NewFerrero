...
public class WebExporlerModule : BaseModule
{
    ...
    //public string loadUrl = "http://172.29.106.254:3000/";
    #endregion
    ...
    protected override void InitEffect()
    {
        ...
        if (webExporlerTrans)
        {
            ...
            webExporlerTrans.gameObject.SetActive(true);
        }
        InitUniWebView();
        UITweener twPos = CreateTweener(topUiTrans, TweenType.TOP_IN);
        SingletonMB<UITweenManagerController>.GetInstance().AddTweener(twPos);
//        twPos = CreateTweener(mainUiTrans, TweenType.TOP_IN);
//        SingletonMB<UITweenManagerController>.GetInstance().AddTweener(twPos);
    }
...
//        SingletonMB<UITweenManagerController>.GetInstance().AddTweener(twPos);
    ...
    private void InitUniWebView()
    {
        if (uniWebView)
        {
            ...
            uniWebView.Load(Singleton<ImageTargetDataModel>.GetInstance().currentUrl);
            Debug.Log("load=" + Singleton<ImageTargetDataModel>.GetInstance().currentUrl);
            uniWebView.Show();
