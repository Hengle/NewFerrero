Shader "Unlit/ARCameraShaderOpenGLSE"
{
	Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            TexCoordInOut vert (Vertex vertex)
            {
                ...
                //o.texcoord = float2(vertex.texcoord.x, -vertex.texcoord.y);
                //if (_isPortrait == 1)
                //{
                ...
                if (_isPortrait == 1)
                {
                    o.texcoord = float2(vertex.texcoord.x -1.0f, -(vertex.texcoord.y - 0.5f) * _texCoordScale + 0.5f);
                }
                else
                {
                    o.texcoord = float2((vertex.texcoord.x - 0.5f) * _texCoordScale - 0.5f, -vertex.texcoord.y);
              ...
              float  rotationRadians=UNITY_PI/2.0;
